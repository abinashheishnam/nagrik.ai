...
def create_complaint(payload: ComplaintCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    c = Complaint(
        ...
        # user doesn't provide category; AI drives it
        category="",
...
def get_all_complaints(db: Session = Depends(get_db)):
    # For admin dashboard - currently public/no-auth as per frontend implementation
...
def my_complaints(db: Session = Depends(get_db), user=Depends(get_current_user)):
    return db.query(Complaint).filter(Complaint.user_id == user.id).order_by(Complaint.id.desc()).all()
